...
A informação acima da linha ```---``` nos diz que os argumento necessários para utilizar o ```/spawn```. ```x```, ```y``` e ```theta``` determinam a posição 2D da tartaruga que acabou de surgir, e ```name``` é um argumento opcional.
